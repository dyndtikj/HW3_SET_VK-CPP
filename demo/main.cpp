 ...
 * in terms of constness */
    void check_constness() {
        ...
        const my_stl::Set<int> s{-4, 5, 3, 0, 7};
        if (s.find(3) == s.end())
        ...
        if (s.size() != 5)
            fail("incorrect size");
        auto first = s.begin();
        my_stl::Set<int>::iterator last = s.end();
        if (*first != -4 || *(--last) != 7)
 ...
 * copy constructor and = operator */
    void check_copy_correctness() {
        ...
        my_stl::Set<int> s1(elems, elems + 10);
...
/* check if class correctly handles empty set */
    void check_empty() {
        ...
        my_stl::Set<std::string> empty;
        my_stl::Set<std::string> s2{"opa"};
        s2 = empty;
        if (s2.size())
            fail("incorrect size");
        my_stl::Set<std::string> s3(s2);
        if (!s3.empty())
            fail("incorrect empty");
        std::cerr << "ok!\n";
    }
...
///* check if class correctly works with iterators */
    void check_iterators() {
        ...
        my_stl::Set<std::pair<int, int> > s{{-3, 5}, {5, 5}, {-4, 1}, {-4, 4}, {0, 1}, {3, 0}};
        if (s.begin()->second != 1 || (++s.begin())->first != -4)
            fail("wrong begin()");
        my_stl::Set<std::pair<int, int> >::iterator cur = s.end();
        my_stl::Set<int> small{1};
...
/* check if class uses only < for elements comparing */
    void check_operator_less() {
        ...
        my_stl::Set<StrangeInt> s{-5, -3, -6, 13, 7, 1000, 963};
...
/* check if class correctly implements destructor */
    void check_destructor() {
        ...
        StrangeInt::init();
        {
            my_stl::Set<StrangeInt> s{5, 4, 3, 2, 1, 0};
            if (s.size() != 6)
                fail("wrong size");
        }
        ...
        {
            my_stl::Set<StrangeInt> s{-3, 3, -2, 2, -1, 1};
            my_stl::Set<StrangeInt> s1(s);
            s1.insert(0);
            my_stl::Set<StrangeInt> s2(s1);
            if (s1.find(0) == s1.end())
                fail("wrong find");
            s1 = s;
            if (s1.find(0) != s1.end())
...
/* check erase for correctness */
    void check_erase() {
        ...
        my_stl::Set<std::string> s{"abacaba", "hello", "p"};
        s.erase("miss");
        s.erase("hello");
        if (s.size() != 2)
            fail("Bad size");
...
int main() {
    ...
    std::cout << "\nAAAA -- my set correctly works -- Hooray!" << std::endl;
    return 0;
}
