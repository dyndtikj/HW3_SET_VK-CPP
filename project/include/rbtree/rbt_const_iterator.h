...
namespace my_rbt {
    namespace iterator {
        ...
        template<typename T>
        my_rbt::rb_node::RBNode<T> *ConstIterator<T>::getPtr() { return ptr_; }
        ...
        template<typename T>
        T const &ConstIterator<T>::operator*() const { return (ptr_->key_); }
        ...
        template<typename T>
        ConstIterator<T>::operator bool() const { return (ptr_ != nullptr); }
